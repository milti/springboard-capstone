...
#walk(searchTwitter(fromAccts[1:10], n=15))
#using this command but now having OAuth errors
#ktweets <- map(acctNames, ~ userTimeline(.x, n=15))

#application of *purrr* function
#latbot <- flatten(flatten(flatten(botlist)))
#newBotlist <- unlist(unname(sapply(flatbot, `[`, 2)))
#botdetails <- map(newBotlist[1:100], ~lookupUsers(.x))

#> for (acct in acctNames) { ktweets <- userTimeline(acct, n=200)}
