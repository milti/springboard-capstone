...
#Test set is for model performance
...
#MAE and RSME (RMSE punishes the errors more severly)
...
#minsplit, cp and maxdepth are important; smaller the cp, more complex the tree
# Plot the "CP Table"
plotcp(grade_model)
...
pred <- predict(object = grade_model_opt,   # model object 
                newdata = grade_test)  # test dataset
...
pred <- predict(object = best_model,
                newdata = grade_test)
rmse(actual = grade_test$final_grade, 
     predicted = pred) #Definite improvement! 2.124
######################################################
#Bagged Trees
#Decision Trees have very high variance, hence bagging
#Average number of trees to reduce variance; reduces overfitting
...
######################################################################
#Random Forest
#Better than bagged trees because of extra randomness added by taking a sub sample of features
#Sounds counter intuitive but you get de-correlated trees
...
#Tuning a RF Model
#ntree - 500
#mtry, sampsize control the variability; sampsize is the number of samples to train on default 63.2%
#nodesize, maxnodes
...
#Boosting
#Adaboost and GBM
#Adaboost - start with training decision tree with equal weights
#Then adjust the weights depending upon how easy it is to classify the observations(increase weight)
#If difficult to classify, lower the weight
#Grow second tree with adjusted weights and so on till maxiterations
#GBM = Gradient Descent + Boosting
#GBM allows you to define your own loss/cost function!; however prone to overfitting
...
#Model Comparison - AUC & ROC Curve
